...
function pagemaster_importapi_importps1() {
	...
	foreach ($lists as $list) {
		...
		$lang => guppy_translate($list['title'])
		));
		$cat->setDataField('display_desc', array (
		$lang => guppy_translate($list['description'])
		));

		$cat->insert();
		$cat->update();
		$dr = $cat->getDataField('id');
		$items = DBUtil :: selectObjectArray("pagesetter_listitems", 'pg_lid = ' . $list['id'], 'pg_lineno');
		foreach ($items as $item) {
			...
			$cat->setDataField('display_name', array (
			$lang => guppy_translate($item['title'])
			));
			$cat->setDataField('display_desc', array (
			$lang => guppy_translate($item['fullTitle'])
			));
			$cat->insert();
			$cat->update();
		}
...
function pagemaster_importapi_importps2() {
	...
	foreach ($pubtypes as $pubtype) {
		...
		$datatype['sortfield1'] = str_replace('pg_lastUpdatedDate','pm_cr_date',$pubtype['sortField1']);
		$datatype['sortdesc1'] = $pubtype['sortDesc1'];
		$datatype['sortfield2'] = str_replace('pg_lastUpdatedDate','pm_cr_date',$pubtype['sortField2']);
		$datatype['sortdesc2'] = $pubtype['sortDesc2'];
		$datatype['sortfield3'] = str_replace('pg_lastUpdatedDate','pm_cr_date',$pubtype['sortField1']);
		$datatype['sortdesc3'] = $pubtype['sortDesc3'];
		...
		for (; !$result->EOF; $result->MoveNext()) {
			...
			elseif ($pubfield['type'] == 'url') $datafield['fieldplugin'] = 'function.pmformurlinput.php';
			elseif ($pubfield['type'] == 'email') $datafield['fieldplugin'] = 'function.pmformemailinput.php';
			...
			elseif ($pubfield['type'] == '0') $datafield['fieldplugin'] = 'function.pmformstringinput.php';
			...
			elseif ($pubfield['type'] == '3') $datafield['fieldplugin'] = 'function.pmformcheckboxinput.php';
			elseif ($pubfield['type'] == '4') $datafield['fieldplugin'] = 'function.pmformintinput.php';
			elseif ($pubfield['type'] == '5') $datafield['fieldplugin'] = 'function.pmformfloatinput.php';
			elseif ($pubfield['type'] == '6') $datafield['fieldplugin'] = 'function.pmformdateinput.php';
			elseif ($pubfield['type'] == '7') //time TODO
			$datafield['fieldplugin'] = 'function.pmformdateinput.php';
			elseif ($pubfield['type'] == '8') $datafield['fieldplugin'] = 'function.pmformurlinput.php';
			elseif ($pubfield['type'] == '9') {
				$datafield['fieldplugin'] = 'function.pmformimageinput.php';
				$datafield['typedata'] = '100:100';
			}
			elseif ($pubfield['type'] == '10') $datafield['fieldplugin'] = 'function.pmformuploadinput.php';
...
function pagemaster_importapi_importps4() {
	...
	$DirPS = pnModGetVar('pagesetter', 'uploadDir');
