...
/**
 * Clip date handler plugin
 */
class Date extends \FilterUtil_AbstractPlugin implements
    \FilterUtil_BuildInterface,
    \FilterUtil_ReplaceInterface
{
    /**
     * Enabled operators.
     *
        ...
        switch ($op) {
            case 'eq':
                if ($type != 'point') {
                    ...
                    $where = "({$column} >= '" . DateUtil::getDatetime($from) . '\' AND ' . "{$column} < '" . DateUtil::getDatetime($to) . '\')';
                } else {
