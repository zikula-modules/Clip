...
/**
 * Clip FilterUtil customization.
 */
class UtilFilter extends \FilterUtil_AbstractBase
{
    /**
    ...
    private function _makeCondition($filter)
    {
        ...
        $obj = array('field' => false, 'op' => false, 'value' => false);
    ...
    public function getFiltersFromInput()
    {
        ...
        $filterStr = FormUtil::getPassedValue($this->_varname, null);
        if (!empty($filterStr)) {
            $filter[] = $filterStr;
        }
        // Get filter1 ... filterN
        while (true) {
            ...
            $filterStr = FormUtil::getPassedValue($filterURLName, null);
    ...
    private function _genDqlRecursive($obj)
    {
        ...
        return array('where' => empty($where) ? '' : "({$where})", 'params' => $params);
    }
