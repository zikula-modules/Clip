        ...
        if (content) {
            ...
            switch (Zikula.Clip.Container.func)
            {
                ...
                case 'generator':
                    ...
                    var clipcode = '';
                    content = content.sub(/<script id="Generatorcode" type="text\/html">([\s\S\/]+)<pre class="clip-generatorcode">/, function (match) {
                        clipcode = match[0].sub(/<script id="Generatorcode" type="text\/html">/, '').sub(/<\/script>([\s\S]+)$/, '');
