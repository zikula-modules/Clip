...
class ParserWorkflow extends \\Zikula_AbstractBase
{
    ...
    public function parse(
        ...
        if (!xml_parse($this->parser, $xmldata, true)) {
            ...
            z_exit($this->__f('Unable to parse XML workflow (line %1$s, %2$s): %3$s.', array(xml_get_current_line_number($this->parser), xml_get_current_column_number($this->parser), xml_error_string($this->parser))));
        }
        // close parser
        xml_parser_free($this->parser);
        // check for errors
        ...
        $this->workflow['workflow'] = array_merge(array('id' => $schemaName, 'module' => $module), $this->workflow['workflow']);
    ...
    public function mapWorkflow()
    {
        ...
        foreach ($this->workflow['states'] as $state) {
            $stateMap[$state['id']] = array('id' => $state['id'], 'title' => $state['title'], 'description' => $state['description']);
        }
        ...
        foreach ($this->workflow['actions'] as $action) {
            if ($action['state'] == 'initial' || $action['state'] == null || in_array($action['state'], $states)) {
                ...
                LogUtil::registerError($this->__f('Unknown %1$s name \'%2$s\' in action \'%3$s\'.', array('state', $action['state'], $action['title'])));
            }
        }
        ...
        foreach (array_keys($this->workflow['actions']) as $stateID) {
            foreach ($this->workflow['actions'][$stateID] as $action) {
                if (isset($action['nextState']) && !isset($this->workflow['states'][$action['nextState']])) {
                    return LogUtil::registerError($this->__f('Unknown %1$s name \'%2$s\' in action \'%3$s\' of the \'%4$s\' state.', array('next-state', $action['nextState'], $action['title'], $action['state'])));
                }
                foreach ($action['operations'] as $operation) {
                    if (isset($operation['parameters']['nextState'])) {
                        ...
                        if (!isset($this->workflow['states'][$stateName])) {
                            return LogUtil::registerError($this->__f('Unknown state name \'%1$s\' in action \'%2$s\', operation \'%3$s\'.', array($stateName, $action['title'], $operation['name'])));
                        }
                    }
                }
            }
    ...
    public function startElement(
        ...
        switch ($state) {
            ...
            case 'error':
                ...
                break;
            default:
                $this->workflow['errorMessage'] = $this->__f('Workflow state error: \'%2$s\' tag of the state \'%1$s\'.', array($state, $name));
                $state = 'error';
                break;
        }
    }
            ...
            case 'workflow':
                switch ($name) {
                    ...
                    case 'DESCRIPTION':
                        ...
                        break;
                    case 'DOMAIN':
                        $this->workflow['workflow']['domain'] = $this->workflow['value'];
                        break;
                }
                break;
            case 'state':
