 ...
 * Clip Installer.
 */
class ClipInstaller extends \Zikula_AbstractInstaller
{
    /**
     * Clip installation
     */
    ...
    {
        // create tables
        $tables = array('Clip_Model_Pubfield', 'Clip_Model_Pubtype', 'Clip_Model_Grouptype', 'Clip_Model_Pubrelation', 'Clip_Model_WorkflowVars');
        ...
        // modvars
        $modvars = array('uploadpath' => $dirs['upload'], 'modelspath' => $dirs['models'], 'pubtype' => null, 'shorturls' => 'htm', 'maxperpage' => 100, 'commontpls' => false, 'devmode' => true);
        ...
        switch ($oldversion) {
            ...
            case '0.4.15':
                ...
                foreach ($regtables as $rtable => $rfield) {
                    DBUtil::executeSQL("DELETE FROM {$rtable} WHERE {$rfield} = 'Clip'");
                }
    ...
    private function createCategoryTree($regpath = '/__SYSTEM__/Modules')
    {
        ...
        $args = array('prop' => 'Global', 'cid' => $c['id']);
        ...
        if (!$c) {
            ...
            $args = array('cid' => $c['id'], 'name' => 'Clip', 'dname' => array($lang => $this->__('Clip')), 'ddesc' => array($lang => $this->__('Clip root category')));
        ...
        if (!$c) {
            $c = CategoryUtil::getCategoryByPath($regpath . '/Clip');
            $args = array('cid' => $c['id'], 'name' => 'Topics', 'dname' => array($lang => $this->__('Topics')), 'ddesc' => array($lang => $this->__('Clip topics for its publications')));
            if (!$this->createCategory($args)) {
                return false;
            }
        }
        ...
        $c = CategoryUtil::getCategoryByPath($regpath . '/Clip/Topics');
        if ($c) {
            $args = array('cid' => $c['id'], 'name' => 'Zikula', 'dname' => array($lang => 'Zikula'), 'ddesc' => array($lang => $this->__('Zikula related publications')), 'leaf' => 1);
            $this->createCategory($args);
            $args = array('cid' => $c['id'], 'name' => 'FreeSoftware', 'dname' => array($lang => $this->__('Free Software')), 'ddesc' => array($lang => $this->__('Free software related publications')), 'leaf' => 1);
            $this->createCategory($args);
            $args = array('cid' => $c['id'], 'name' => 'Community', 'dname' => array($lang => $this->__('Community')), 'ddesc' => array($lang => $this->__('Community related publications')), 'leaf' => 1);
            $this->createCategory($args);
            // create the Category Registry
            // create an entry in the categories registry to the Topics property
            $args = array('prop' => 'Topics', 'cid' => $c['id']);
            $this->createCategoryRegistry($args);
        } else {
    ...
    private function createDirectories($toCreate = array(), $silent = false)
    {
        ...
        foreach ($toCreate as $name) {
            $dir = $dirs[$name];
            if (!file_exists($dir) && mkdir($dir, System::getVar('system.chmod_dir', 511), true)) {
    ...
    private static function renameClipTables()
    {
        ...
        $tables['clip_relations'] = 'clip_relations';
        $tables['clip_pubfields'] = 'clip_pubfields';
        $tables['clip_pubtypes']  = 'clip_pubtypes';
        ...
        foreach ($pubtypes as $tid) {
            if (in_array(DBUtil::getLimitedTablename('pagemaster_pubdata' . $tid), $existingtables)) {
                ...
                $tables['clip_pubdata' . $tid] = 'clip_pubdata' . $tid;
                $serviceManager['dbtables'] = array_merge($dbtables, (array) $tables);
    ...
    private function renameRoutine()
    {
        ...
        $plugins = array('Checkbox' => array('Clip_Form_Plugin_Checkbox', 'pmformcheckboxinput'), 'Date' => array('Clip_Form_Plugin_Date', 'pmformdateinput'), 'Email' => array('Clip_Form_Plugin_Email', 'pmformemailinput'), 'Float' => array('Clip_Form_Plugin_Float', 'pmformfloatinput'), 'Image' => array('Clip_Form_Plugin_Image', 'pmformimageinput'), 'Int' => array('Clip_Form_Plugin_Int', 'pmformintinput'), 'List' => array('Clip_Form_Plugin_List', 'pmformlistinput'), 'Ms' => array('Clip_Form_Plugin_Ms', 'pmformmsinput'), 'MultiCheck' => array('Clip_Form_Plugin_MultiCheck', 'pmformmulticheckinput'), 'MultiList' => array('Clip_Form_Plugin_MultiList', 'pmformmultilistinput'), 'Pub' => array('Clip_Form_Plugin_Pub', 'pmformpubinput'), 'String' => array('Clip_Form_Plugin_String', 'pmformstringinput'), 'Text' => array('Clip_Form_Plugin_Text', 'pmformtextinput'), 'Upload' => array('Clip_Form_Plugin_Upload', 'pmformuploadinput'), 'Url' => array('Clip_Form_Plugin_Url', 'pmformurlinput'), 'RadioList' => array('Clip_Form_Plugin_RadioList'));
        ...
        $ptcols = Doctrine_Core::getTable('Clip_Model_Pubtype')->getFieldNames();
    ...
    private function migratePubField()
    {
        ...
        $this->setVar('modelspath', $dirs['models']);
        ZLoader::addAutoloader('ClipModels', realpath(StringUtil::left($dirs['models'], -11)));
        $pubtypes = Doctrine_Core::getTable('Clip_Model_Pubtype')->selectFieldArray(
            'folder',
            null,
            '',
            false,
        ...
        foreach ($fields as $field) {
            ...
            $relations[$tid1][$tid2][$key]['info'] = array('title' => $field['title'], 'description' => $field['description']);
        }
        // 2. Create the new relations
        foreach ($relations as $tid1 => $y) {
            foreach ($y as $tid2 => $x) {
                foreach ($x as $fieldname => $v) {
                    $reldata = array('tid1' => $tid1, 'alias1' => $fieldname, 'title1' => $v['info']['title'], 'descr1' => $v['info']['description'], 'tid2' => $tid2, 'alias2' => $pubtypes[$tid1] . '_' . $fieldname, 'title2' => $v['info']['title']);
            ...
            foreach ($y as $tid2 => $x) {
                foreach ($x as $fieldname => $v) {
                    if (!isset($dbfields[$tid1])) {
                        $dbfields[$tid1] = Doctrine_Core::getTable('ClipModels_Pubdata' . $tid1)->getFieldNames();
                    }
                    ...
                    if (count($v['ids']) > 1) {
                        ...
                        foreach ($v['ids'] as $fid => $name) {
                            ...
                            $where = array($name . ' IS NOT NULL', array($name . ' != ?', 0), array('core_online = ?', 1));
                            $ids = $tbl1->selectFieldArray(
                                $name,
                                $where,
                                '',
                                false,
                            ...
                            if ($ids) {
                                $where = array(array('core_pid IN ?', array_unique(array_values($ids))), array('core_online = ?', 1));
                                $pids = $tbl2->selectFieldArray(
                                    'id',
                                    $where,
                                    '',
                                    false,
                        ...
                        foreach ($v['ids'] as $fid => $name) {
                            ...
                            $q = $tbl1->createQuery();
                            $q->update()->set($name, 'NULL')->where($name . ' = 0')->execute();
                            $where = array($name . ' IS NOT NULL', array('core_online = ?', 1));
                            $ids = $tbl1->selectFieldArray(
                                $name,
                                $where,
                                '',
                            ...
                            if ($ids) {
                                $where = array('whereIn' => array('core_pid', array_unique(array_values($ids))), array('core_online = ?', 1));
                                $pids = $tbl2->selectFieldArray(
                                    'id',
                                    $where,
                                    '',
                                ...
                                foreach ($ids as $id => $pid) {
                                    ...
                                    $q = $tbl1->createQuery();
                                    $q->update()->set($name, $pids[$pid])->where('id = ?', $id)->execute();
                                }
                            }
                            // rename the field
    ...
    private static function upgTablePubtypes()
    {
        ...
        if (in_array('pm_inputset', $ptcols)) {
    ...
    private function upgradeDBpre09()
    {
        ...
        // grouptypes
        if (in_array('clip_grouptypes', $tables)) {
        ...
        // relations
        $rlcols = Doctrine_Core::getTable('Clip_Model_Pubrelation')->getFieldNames();
    ...
    private function introduceUrltitle()
    {
        ...
        foreach ($pubtypes as $tid) {
            ...
            $titles = Doctrine_Core::getTable('ClipModels_Pubdata' . $tid)->selectFieldArray(
                $titlefield,
                array(),
                '',
                false,
