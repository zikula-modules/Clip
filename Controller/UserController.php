...
class UserController extends \Zikula_AbstractController
{
    ...
    public function viewAction($args)
    {
        ...
        $apiargs['where'][] = array('(core_publishdate IS NULL OR core_publishdate <= ?)', date('Y-m-d H:i:s', time()));
        $apiargs['where'][] = array('(core_expiredate IS NULL OR core_expiredate >= ?)', date('Y-m-d H:i:s', time()));
    ...
    public function editAction($args)
    {
        ...
        if ($args['id']) {
            $pubdata = ModUtil::apiFunc('Clip', 'user', 'get', array('tid' => $args['tid'], 'id' => $args['id'], 'templateid' => $args['templateid'], 'checkperm' => true, 'handleplugins' => false, 'loadworkflow' => false));
    ...
    public function execAction($args)
    {
        ...
        // define the arguments
        $args = array('tid' => $pubtype['tid'], 'id' => isset($args['id']) ? $args['id'] : FormUtil::getPassedValue('id'), 'action' => isset($args['action']) ? $args['action'] : $args['commandName'], 'goto' => isset($args['goto']) ? $args['goto'] : FormUtil::getPassedValue('goto'));
        //// Validation
