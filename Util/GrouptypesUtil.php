    ...
    public static function getTreeJSNode($grouptype, &$leafNodes)
    {
        ...
        if (isset($grouptype['pubtypes']) && $grouptype['pubtypes']) {
            foreach ($grouptype['pubtypes'] as $pubtype) {
                ...
                $grouptype['nodes'][$id] = array('href' => ModUtil::url('Clip', 'admin', 'pubtypeinfo', array('tid' => $pubtype['tid'])), 'name' => DataUtil::formatForDisplay($pubtype['title']), 'title' => DataUtil::formatForDisplay($pubtype['description']));
                $leafNodes[] = $id;
            }
