...
class ViewForm extends \Zikula_Form_View
{
    ...
    public function execute($template, Zikula_Form_AbstractHandler $eventHandler)
    {
        ...
        if ($this->isPostBack()) {
            if (!SecurityUtil::validateCsrfToken($this->request->getPost()->filter(
                'csrftoken',
                '',
            ...
            // retrieve form id
            $formId = $this->request->getPost()->filter(
                '__formid',
                '',
                FILTER_SANITIZE_STRING
            );
    ...
    public function postRender()
    {
        if (ModUtil::available('Scribite')) {
            ...
            if (!empty($ids)) {
                $args = array('modulename' => 'Clip', 'editor' => $editor, 'areas' => $ids);
